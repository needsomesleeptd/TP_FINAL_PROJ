...
def get_container_name(item: dict) -> [str, str]:
    ...
    if owner and repository and tag:
        return f'{owner}/{repository}:{tag}', re.sub(r".*-prod_", "", tag)
    if repository and tag:
        return f'{repository}:{tag}', re.sub(r".*-prod_", "", tag)
    return '', ''
...
def deploy_new_container(image_name: str, container_name: str, ports: dict = None):
    try:
        ...
        # Запуск нового контейнера заметьте что сеть для подключения захардкожена
        docker_client.containers.run(image=image_name, name=container_name, detach=True, ports=ports,network ="meet_match_default")
    except Exception as e:
