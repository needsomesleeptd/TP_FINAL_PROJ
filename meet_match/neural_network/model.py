...
class RecommendationSystem:
    ...
    def _get_swipes_for_session(
        ...
        swipes_list = [(place_id, bool(is_liked)) for place_id, is_liked in swipes]
    ...
    def _get_rec_on_query(
        self, query: str, used_ids: List[int], num_items: int = 20
    ) -> List[int]:
        """
    ...
    def get_rec(
        self, user_id: int, session_id: int, query: str, num_idx=50
    ) -> List[int]:
        """
        Параметры:
        - user_id (int): Идентификатор пользователя.
        ...
        final_recomendation = (
            query_rec_list + rec_user_hist + rec_group_hist + random_rec
        )
    ...
    def build_embeddings(self):
        conn = pg.connect(**self.connection_info)
        cursor = conn.cursor()
        cursor.execute("SELECT place_id, title, description FROM places")
